...
result_path=fairseq/examples/wav2vec/outputs/decode/ft10m
...
for lm_weight in $(seq 2 0.2 4); do
    for word_score in $(seq -1 0.1 0); do
        mkdir -p ${workdir}/fairseq/examples/wav2vec/outputs/lmweight${lm_weight}_wordscore${word_score}
        python ${workdir}/fairseq/examples/speech_recognition/infer.py \
            ...
            --nbest 1 --path /userhome/user/zsz01/repo/fairseq/examples/wav2vec/outputs/10m-4-gram-wav2vec_small/checkpoints/checkpoint_best.pt \
            --gen-subset ${subset} \
            ...
            --max-tokens 4000000 \
            --post-process letter \
            --beam 1500  2> ${workdir}/fairseq/examples/wav2vec/outputs/lmweight${lm_weight}_wordscore${word_score}/decode-10m.log
    done
done
