...
result_path=fairseq/examples/wav2vec/outputs/decode/ft100h
...
for lm_weight in $(seq 1.5 0.1 2.5); do
    for word_score in $(seq -2 0.2 0); do
        ...
        python ${workdir}/fairseq/examples/speech_recognition/infer.py \
            ...
            --nbest 1 --path /userhome/user/zsz01/repo/fairseq/examples/wav2vec/outputs/fine-tune/100h/100h-None-wav2vec_small/checkpoints/checkpoint_best.pt \
            --gen-subset ${subset} \
            ...
            --max-tokens 4000000 \
            --post-process letter \
            --beam 1500  2> ${workdir}/${result_path}/lmweight${lm_weight}_wordscore${word_score}/decode-100h.log
    done
done
