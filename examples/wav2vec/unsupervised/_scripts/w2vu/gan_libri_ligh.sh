...
TASK_DATA=${FAIRSEQ_ROOT}/examples/wav2vec/manifest-u/processed_audio/precompute_pca512_cls128_mean_pooled
TEXT_DATA=${FAIRSEQ_ROOT}/examples/wav2vec/manifest-u/processed_text/phones
KENLM_PATH=${FAIRSEQ_ROOT}/examples/wav2vec/manifest-u/processed_text/phones/lm.phones.filtered.06.bin
output_dir=${work_dir}/outputs/libri-light/matched/seed$1
...
for code in ${code_penalty}; do
    for gradient in ${gradient_penalty}; do
        for smooth in ${smoothness_weights}; do
            ...
            common.wandb_project=w2vu-libri-light-matched \
