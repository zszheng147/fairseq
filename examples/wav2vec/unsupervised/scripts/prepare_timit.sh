...
find $timit_root/{TRAIN,TEST} -iname "*.WAV" > $tgt_dir/all_sph.flist #; collect all wav path
cat $tgt_dir/all_sph.flist | sed -e 's#//*#/#g' -e 's#.*/\([^/]*\)/\([^/]*\).WAV#\1_\2#g' > $tgt_dir/all.uid
#; /home/data/TIMIT/TRAIN/DR3/FSJW0/SA2.WAV ----> FSJW0_SA2
paste -d' ' $tgt_dir/{all_sph.flist,all.uid} | \
  awk -v sph2wav=$sph2wav -v wav_dir=$wav_dir '{print sph2wav " -f wav " $1 " > " wav_dir "/" $2 ".wav"}' \
  > $tgt_dir/sph2wav.sh     #; format tranformation: .WAV --> .wav
bash $tgt_dir/sph2wav.sh
cat $tgt_dir/all.uid | awk -v wav_dir=$wav_dir '{print $1" "wav_dir"/"$1".wav"}' | sort > $tgt_dir/all_wav.scp
#; pair (uid sph_path) FSJW0_SX433 /fairseq/examples/wav2vec/manifest-u/timit/wav/FSJW0_SX433.wav
cut -d' ' -f2 $tgt_dir/all_wav.scp | xargs -I{} soxi -s {} > $tgt_dir/all.dur #; number of sample of each wav
...
#; h# --> sil; at the same time, align uid and phn sequence
...
for s in $setups; do
  zsh scripts/prepare_audio_v2.sh $tgt_dir/$s $tgt_dir/$s/feat $model

  lm_dir=$tgt_dir/$s/phones
  fst_dir=$tgt_dir/$s/fst/phn_to_phn

  python $FAIRSEQ_ROOT/fairseq_cli/preprocess.py --dataset-impl mmap --trainpref $tgt_dir/$s/train_text.phn --workers 10 --only-source \
     --destdir $lm_dir --srcdict $tgt_dir/$s/dict.phn.txt
