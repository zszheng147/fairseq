...
for code in ${code_penalty}; do
    for gradient in ${gradient_penalty}; do
        for smooth in ${smoothness_weights}; do
            export WANDB_NAME=seed$1-code${code}-gradient${gradient}-smooth${smooth}-bsz640
            if [ -d ${output_dir}/code${code}-gradient${gradient}-smooth${smooth}-bsz640 ]; then
                echo "This group of hyperparameters have been trained."
                continue
            else
                mkdir -p ${output_dir}/code${code}-gradient${gradient}-smooth${smooth}-bsz640
            fi
            ...
            checkpoint.save_dir=${output_dir}/code${code}-gradient${gradient}-smooth${smooth}-bsz640/checkpoints \
            common.tensorboard_logdir=${output_dir}/code${code}-gradient${gradient}-smooth${smooth}-bsz640/tensorboard \
            hydra.run.dir=${output_dir}/code${code}-gradient${gradient}-smooth${smooth}-bsz640 \
            ...
            dataset.batch_size=640 \
            optimizer.groups.generator.lr=[0.0001] \
            optimizer.groups.discriminator.lr=[0.0006] \
            'common.seed='$1'' 
        done
    done
done