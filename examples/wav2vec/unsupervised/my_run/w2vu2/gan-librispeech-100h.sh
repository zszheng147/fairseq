...
TASK_DATA=$manifest/processed-feat-100h
TEXT_DATA=$manifest/processed-lm-from-librivox/phones
KENLM_PATH=$manifest/processed-lm-from-librivox/phones/lm.phones.filtered.06.bin
...
for code in ${code_penalty}; do
    for gradient in ${gradient_penalty}; do
        for smooth in ${smoothness_weights}; do
            ...
            common.wandb_project=w2vu2-librispeech-100h \
            ...
            if [ -d ${FAIRSEQ_ROOT}/transcriptions/w2vu2/librispeech-100h/seed$1/code${code}-gradient${gradient}-smooth${smooth} ]; then
            ...
            hydra.run.dir=${FAIRSEQ_ROOT}/transcriptions/w2vu2/librispeech-100h/seed$1/code${code}-gradient${gradient}-smooth${smooth} 
