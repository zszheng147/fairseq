...
TEXT_DATA=$manifest/raw-openslr-100k/letters
KENLM_PATH=$manifest/raw-openslr-100k/letters/lm.letters.06.bin
output_dir=${FAIRSEQ_ROOT}/outputs/w2vu2/letters-960h-openslr/seed$1
    ...
    for gradient in ${gradient_penalty}; do
        for smooth in ${smoothness_weights}; do
            for mmi in ${mmi_weights}; do
                ...
                task.labels=$labels \
                model.segmentation.labels=$labels \
